...
def add_red_cloth(image):
    ...
    for (x, y, w, h) in faces:
        # Eye region
        eye_region_height = int(h / 5)
        eye_y_start = y + int(h / 4)
        eye_y_end = eye_y_start + eye_region_height
        cv2.rectangle(image, (x, eye_y_start), (x+w, eye_y_end), (0, 0, 255), -1)
        
        # Mouth region
        mouth_region_height = int(h / 4)
        mouth_y_start = y + int(2 * h / 3)
        mouth_y_end = mouth_y_start + mouth_region_height
        cv2.rectangle(image, (x, mouth_y_start), (x+w, mouth_y_end), (0, 0, 255), -1)
