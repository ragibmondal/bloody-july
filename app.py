...
def add_red_cloth(image):
    ...
    # Detect faces
    gray = cv4.cvtColor(image, cv2.COLOR_BGR2GRAY)
    faces = face_cascade.detectMultiScale(gray, 1.3, 5)
    
    for (x, y, w, h) in faces:
        # Define regions for eyes and mouth with reduced size
        region_height = int(h / 6)
        region_width = int(w * 0.5)
        
        # Cover eyes
        eye_y_start = y + int(h / 5)
        eye_y_end = eye_y_start + region_height
        eye_x_start = x + int((w - region_width) / 2)
        eye_x_end = eye_x_start + region_width
        image[eye_y_start:eye_y_end, eye_x_start:eye_x_end] = [0, 0, 255]
        
        # Cover mouth
        mouth_y_start = y + int(2 * h / 3)
        mouth_y_end = mouth_y_start + region_height
        mouth_x_start = x + int((w - region_width) / 2)
        mouth_x_end = mouth_x_start + region_width
...
def main():
    ...
    st.write("Upload an image to add red cloth over eyes and mouth without changing the image quality.")
